...
public class ClasseGeneriqueUniteMatiereModuleAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			// éviter la duplication d'une module dans d'autre unités ou d'autres matières
//			if(configurationModuleDAO.exists(configurationModule)== null){
				configurationModuleDAO.insert(configurationModule);
				success=true;
				warning = false;
				message="Le module a élé ajouté avec succès à la matière";
...
//			}
			...
			success=false;
			warning = false;
			message="Impossible d'ajouter le module à la matière ";
			e1.printStackTrace();
		}
