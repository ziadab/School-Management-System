...
public class ClasseGeneriqueUniteMatiereAdd extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			//TODO : ce test est incomplet : ( maitenant il evite de suppliquer une matière dans 
			// la meme unité ) Mais .. à corriger pour 
			// éviter la duplication d'une matière dans d'autre unités de la meme classes génériques
			if(configurationMatiereDAO.exists(configurationMatiere)== null){
				configurationMatiereDAO.insert(configurationMatiere);
				message="La matière a élé ajoutée avec succès à l'unité ";
				
			}else{
				message="Cette matière existe déja dans l'une des unités ";
			}
		} catch (SQLException e1) {
			message="Impossible d'ajouter la matière à l'unité ";
			e1.printStackTrace();
		}
