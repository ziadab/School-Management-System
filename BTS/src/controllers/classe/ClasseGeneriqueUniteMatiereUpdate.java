	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if(configurationMatiereDAO.exists(configurationMatiere)!= null && configurationMatiereDAO.exists(configurationMatiere).getId() != configurationMatiere.getId()){
				message="Impossible de modifier la matière . Cette matière existe déja dans l'une des unités ";
			}else{
				configurationMatiereDAO.update(configurationMatiere);
				message="La modification de la matière élé faite avec succès";
				
			}	

			
		} catch (SQLException e1) {
			message="Impossible de modifier la configuration de la matière";
			e1.printStackTrace();
		}
