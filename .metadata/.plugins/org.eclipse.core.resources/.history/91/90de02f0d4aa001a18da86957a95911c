	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String photoEtudiantDoc = null;
   	 	if(photoEtudiant!="" ){
	   	    ...
	   	    // Si la photo n'a pas été modifiée : conserver la meme photo dans la base de données
	        if (f1.exists()){
	        ...
	        } else // sinon sauvegarder la nouvelle photo
	        {
				String savePath=storagePath+File.separator+uploadDir+ File.separator + photoEtudiant;
			   	...
			   	if(photoEtudiantTemp.exists()){
			   		//Enlever les caratère accetués qui peuvent causé des problème
		...
		try {
			...
			result="L'étudiant à été modifié avec succès";
		} catch (SQLException e) {
			result="Impossible de modifier l'étudiant";
			e.printStackTrace();
		}
