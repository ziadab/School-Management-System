...
<html>
  <head>
  	<title>Détail du controle</title>
		     ...
		     <div class="panel-heading">
					<nav class="nav nav-tabs flex-column flex-sm-row"  id="Tab-Content" role="tablist">
					    <a id="tab_Etudiants" href="#content-tab-Etudiants" class="nav-item nav-link active" data-toggle="tab" role="tab" aria-controls="content-tab-Etudiants" aria-selected="false">
					        <i class="fa fa-users mr-2"></i> Notes
					    </a>
					    <a id="tab_EmploieDuTemps" href="#content-tab-EmploieDuTemps" class="nav-item nav-link" data-toggle="tab" role="tab" aria-controls="content-tab-EmploieDuTemps" aria-selected="false">
					        <i class="fa fa-calendar-alt mr-2"></i> something1
					    </a>
					    <a id="tab_Absences" href="#content-tab-Absencess" class="nav-item nav-link" data-toggle="tab" role="tab" aria-controls="content-tab-Absences" aria-selected="false">
					        <i class="fa fa-user-check mr-2"></i> something2
					    </a>
					    <a id="tab_Evaluations" href="#content-tab-Evaluations" class="nav-item nav-link" data-toggle="tab" role="tab" aria-controls="content-tab-Evaluations" aria-selected="false">
					        <i class="fa fa-file-signature mr-2"></i> something3
					    </a>
					</nav>
			</div>
		...
		// Add event listener for editing notes
  		...
  		// Initialisation du DataTable controle
			...
			function populateControleDataTable(){
				...
				//tableData.DataTable().rows().remove().draw();		// Vider la TableData 
				$.ajax({
					url : "../Controle/List?classeId="+<%= request.getParameter("id")%>,
					type: "GET",
					dataType: 'json',
					success: function(response,textStatus ,jqXHR){
						for(i=0;i<response.length;i++){
							...
							var btns='<div class="btn-group dropleft"> \
								  ...
								  <div class="dropdown-menu shadow p-3 mb-5 bg-white rounded" id="dropdownCycle"> \
							        ...
							        <div class="dropdown-divider"></div> \
								  	...
								  	<div class="dropdown-divider"></div> \
								  	<a class="dropdown-item classe-full-details text-primary" href="${pageContext.request.contextPath}/E.Professeur/ClasseDetails?id='+response[i].id+'" classeId="'+response[i].id+'"><span class="fa fa-cog mr-4"></span>Détails</a> \
								  </div> \
								</div> ';
...
/********** Ajouter un nouveau Controle **********************/
  			...
  			$('#controle_Add_Modal').on('shown.bs.modal', function fillAddForm(evt){
  				...
  				$.ajax({
					...
					success: function(response,textStatus ,jqXHR){
						...
						//charger la valeur du module aver -1 dans le debut d'affichage du form
						var item  ='<option value="-1">null</option>';
						$("#module").append(item);
				    },
				    error: function(response,textStatus ,jqXHR){
				    	$("#modalError .modal-body p").html("");
				...
				$("#matiere").change(function(){
	  				...
	  				$.ajax({
						url : "../ModuleByMatiere/List?matiereId="+matiereId,
						type: "GET",
						dataType: 'json',
						contentType: "application/json; charset=UTF-8",
						success: function(response,textStatus ,jqXHR){
							...
							for(i=0;i<response.length;i++){
								...
								var item  ='<option value="'+response[i].module.id+'">'+response[i].module.nom_Fr+'</option>';
								$("#module").append(item);       
							}	}
					    },
					    error: function(response,textStatus ,jqXHR){
  			...
  			$('#data_table_controle').on('click','.controle-details', function(evt){
				...
				$.ajax({
					...
					success: function(response,textStatus ,jqXHR){
						...
						var classeItem ='<li class="list-group-item"><span><strong>Classe	</strong></span><span class="float-right">'+<%= request.getParameter("id")%>+'</span></li>';
